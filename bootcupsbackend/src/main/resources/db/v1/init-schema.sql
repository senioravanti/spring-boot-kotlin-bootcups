...
(
    ...
    'Горячий суп с гарниром, в котором основным ингредиентом является лапша'),
(
...
WHERE
    ...
    FROM categories
WHERE
    category_name = 'Здоровое питание')),
((
    SELECT
        product_id
    FROM products
...
WHERE
    ...
    FROM categories
WHERE
    category_name = 'Напитки из цикория')),
((
    SELECT
        product_id
    FROM products
...
WHERE
    ...
    FROM categories
WHERE
    category_name = 'Хлеб на закваске'));
